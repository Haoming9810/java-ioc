...
public class FoodOrderingService {
    ...
    public void order(String client, String food) {
        ...
        if (id == null) {
            System.out.printf("Sorry, %s is not on the menu.", food);
            return;
        }
        database.write(client, id);
        System.out.printf("Food %s (%s) ordered by %s", food, id, client);
    }

    public void checkStatus(String client) {
        String id = database.read(client);
        if (id == null) {
            System.out.printf("Error: no order by %s", client);
            return;
        }
        System.out.printf("%s's order %s is ready for pickup!", client, foodName(id));
    }
